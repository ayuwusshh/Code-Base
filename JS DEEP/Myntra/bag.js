...
function displayBagItems() {
    ...
    bagItemObjects.forEach(item => {
        ...
        `;
        bagItemsContainer.innerHTML += itemHTML;
    });
}

function removeFromBag(itemId) {
    // Remove item from bagItems array and localStorage
    bagItemObjects = bagItemObjects.filter(item => item.id !== itemId);
    localStorage.setItem('bagItems', JSON.stringify(bagItemObjects.map(item => item.id)));
    
    // Update the displayed items and summary
    displayBagItems();
    displayBagSummary();
}
